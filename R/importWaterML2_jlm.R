...
#' 
importWaterML2Jlm <- function(obs_url, asDateTime=FALSE, tz=""){
  queryTime <- Sys.time()
...
#' @export
ParseWaterML2 <- function(import, asDateTime=FALSE, tz="", 
...
## unclear what this is trying to do. seems like this would rarely be zero length. but most of the 
##entries might be
...
#########################################
    ...
    if("swe:value" %in% names(DF2)){
      ...
      ## my example dosent need the rep, which is default anyway for scalar assingment
      DF2$qualifier <- defaultQualifier 
    }
